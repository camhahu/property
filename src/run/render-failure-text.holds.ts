...
export const renderFailureTextSpec = spec(renderFailureText, ({ law, section }) => {
    section("output", () => {
        law("includes the law name reason and source", {
            holds: ({ input, result }) =>
        ...
        law("numbers calls starting at one", {
            where: ({ input }) => input.calls.length > 1,
            holds: ({ result }) => result.includes("  call 1:") && result.includes("  call 2:"),
        });

        law("starts the first call immediately after the header gap", {
            where: ({ input }) => input.calls.length > 0,
            holds: ({ input, result }) => result.startsWith(`  ${input.lawName}\n\n  call 1:`),
        });

        law("separates later calls with a blank line", {
            where: ({ input }) => input.calls.length > 1,
            holds: ({ result }) => result.includes("\n\n  call 2:"),
        });

        law("renders result calls separately from error calls", {
            where: ({ input }) =>
        ...
        law("renders object result keys when present", {
            where: ({ input }) => renderedResultKeys(input).length > 0,
            holds: ({ input, result }) => {
                const renderedResultSection = firstResultSection(result);

                return renderedResultKeys(input).every((key) =>
                    renderedResultSection.includes(key),
                );
            },
        });

        law("sorts simple object result keys before rendering", {
        ...
        law("renders nested result values beyond the first level", {
            where: ({ input }) => nonEmptyNestedResultKey(input) !== undefined,
            holds: ({ input, result }) =>
                firstResultSection(result).includes(nonEmptyNestedResultKey(input) ?? ""),
        });

        law("includes the law snippet when present", {
            where: ({ input }) => Boolean(input.lawSnippet),
            holds: ({ input, result }) => result.includes(`  law: ${input.lawSnippet}`),
        });
    });
});
